...
#-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
#2、登录boss
curl -s -k -c cookie.ck 'https://xxxx/users/login?username=xxx&password=xxxx' >/dev/null
...
curl -s -k -b cookie.ck 'https://xxxx='$(date -d "7 days ago" +%F)'+00%3A00%3A00&create_time_max='$(date +%F)'+00%3A00%3A00&limit=500' \
	|jq -c '.data[] | {"客户邮箱":.xxxx.email,"子账号":.sub_account_login_name,"cpu":.cpu,"内存":.memory,"系统盘大小-GB":.sys_space,"数据盘大小-GB":.data_space,"磁盘类型":.disk_type,"自动备份周数":.auto_backup_week_num,"自动备份天数":.auto_backup_day_num,"带宽":.networks[1].band_width,"创建时间":.create_time,"服务开始时间":.begin_time,"服务到 期时间":.expire_time,"service_code":.service_code,"Zone":.zone.name}'\
	|awk -F \" '{print $4,"," $8,"," $11,$13,$15,$17,$20,"," $23,$25,$27,$30, "," $34, ","$38, ","$42, ","$46}' \
	|grep -Ev 'xxx@xinnet.com'\
